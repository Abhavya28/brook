...
//             {/* HOVER / MOBILE ACTION */}
//             <div className="absolute inset-0 bg-black/50 opacity-0 hidden md:flex group-hover:opacity-100 transition">
//               <button
//                 onClick={() => addToCart(product)}
//                 className="md:hidden mt-3 bg-primary text-white py-2 w-full"
//               >
//                 Add to Cart
//               </button>
...
//         <p className="text-center text-gray-500 pt-20">No products found</p>
//       )}
    ...
    <div id="shop" className="pt-24">
      ...
      <div className="relative w-full h-80">
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center">
          ...
          <h1 className="text-5xl pb-2">Shop</h1>
          <div className="flex gap-2 items-center">
            <p>Home</p>
            <img src="/images/right-arrow-line.svg" className="h-4 w-4" />
            <p>Shop</p>
          </div>
        </div>
      </div>
      ...
      {/* FILTER BAR */}
      <div className="bg-[#F9F1E7] px-6 md:px-24 py-6 flex flex-col md:flex-row gap-6 justify-between">
        <div className="flex gap-6 items-center flex-wrap">
          ...
          <div className="border-l-2 border-footer pl-4">
            <p>
              Showing {startIndex + 1}–
              {Math.min(endIndex, filteredProducts.length)} of{" "}
              {filteredProducts.length} results
      ...
      {/* PRODUCTS GRID */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8 pt-16 px-6 md:px-16">
        {currentProducts.map((product) => (
          <div
            key={product.id}
            className="bg-[#F4F5F7] relative group overflow-hidden"
              ...
              <div className="flex gap-4">
                {hoverIcons.map((icon, index) => (
                  <a
                    ...
                    className="flex items-center gap-2 text-white text-sm"
                  >
                    <img src={icon.icon} className="w-5 h-5" />
                    {icon.label}
                  </a>
                ))}
              </div>
